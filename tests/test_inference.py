...
def test_inference_determinism(inputs, trained_model_for_inference):
    ...
    if len(user_data_list) > 1:
        ...
        user_emb3 = inference.generate_user_embedding(model, different_user, encoders_used=encoders_used, expected_num_features=user_expected_features)
        ...
        assert emb_diff > 1e-8, \
            ...
            f"User {test_user['user_id']} and User {different_user['user_id']} embeddings differ by only {emb_diff:.10f}"
        ...
        assert similarity_diff > 1e-8, \
            f"Different users produce identical similarities! User {test_user['user_id']} similarity: {similarity1:.8f}, " \
